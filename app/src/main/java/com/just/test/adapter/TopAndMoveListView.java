...
import com.just.test.bean.ItemInfo;
 ...
 *
 */
public class TopAndMoveListView extends ListView {
	...
	@SuppressLint("NewApi")
	public TopAndMoveListView(Context context, AttributeSet attrs) {
		super(context, attrs);
...
//		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_DOWN && !isLock && !isMoving && !isDragItemMoving) {
			...
			MoveAndTopAdapter adapter = (MoveAndTopAdapter) getAdapter();
			...
			// 获取可拖拽的图标
			View draggerIcon = dragger.findViewById(R.id.item_drag_icon);
...
//		scaleAnimation.setDuration(300);
	...
	private void hideDropItem(){
		final MoveAndTopAdapter adapter = (MoveAndTopAdapter)this.getAdapter();
...
//	private List<int[2]> listOrder = new ArrayList<Integer>();
	...
	private void testAnimation(int y){
		final MoveAndTopAdapter adapter = (MoveAndTopAdapter) getAdapter();
		int tempPosition = pointToPosition(0, y);
