...
public class VerticalScrollTextView extends TextView {
	...
	private void init() {
		...
		mPaint.setTextSize(60f);//设置字体大小
		mPaint.setColor(Color.BLACK);//设置字体颜色
		mPaint.setTypeface(Typeface.SERIF);//字体风格，包括粗体，斜体以及衬线体，非衬线体等
		//1，noraml （普通字体,系统默认使用的字体）2，sans（非衬线字体）3，serif （衬线字体）4，monospace（等宽字体）

		// 高亮部分 当前歌词
		mPathPaint = new Paint();
		mPathPaint.setAntiAlias(true);
		mPathPaint.setColor(Color.RED);
		mPathPaint.setTextSize(80f);
		mPathPaint.setTypeface(Typeface.SANS_SERIF);
	}
	protected void onDraw(Canvas canvas) {
		super.onDraw(canvas);
		canvas.drawColor(0xEFeffff);//设置画布的背景颜色
		Paint p = mPaint;
		Paint p2 = mPathPaint;
	...
	class updateThread implements Runnable {
		//// TODO: 2016/12/2 每次停顿的时间
		long time = 500; // 开始 的时间，不能为零，否则前面几句歌词没有显示出来
		int i=0;
		public void run() {
			while (true) {
				long sleeptime = updateIndex(i);
//				time += sleeptime;//可使滚动的速度越来越慢
