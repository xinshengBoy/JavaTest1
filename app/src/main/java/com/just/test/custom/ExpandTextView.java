...
public class ExpandTextView extends TextView implements View.OnClickListener {
    ...
    public ExpandTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        arrowDrawablePadding = (int) typedArray.getDimension(R.styleable.ExpandTextView_arrowPadding, DensityUtil.dip2px(context, 2f));

        typedArray.recycle();

        if (mExpandDrawable == null) {
            mExpandDrawable = getDrawable(getContext(), R.drawable.sign_down);
        }
        if (mCollapseDrawable == null) {
            mCollapseDrawable = getDrawable(getContext(), R.drawable.sign_up);
        }
