...
public class DashboardView1 extends View {
    ...
    private void init() {
        mStrokeWidth = dp2px(1);// 画笔宽度
        mLength1 = dp2px(8) + mStrokeWidth;// 长刻度的相对圆弧的长度
        mLength2 = mLength1 + dp2px(2);// 刻度读数顶部的相对圆弧的长度
        mPSRadius = dp2px(10);// 指针短半径
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mRadius = (width - mPadding * 2 - mStrokeWidth * 2) / 2;// 扇形半径

        mPaint.setTextSize(sp2px(16));//中间实时指数的文字大小
        ...
        // 由起始角度确定的高度
        float[] point1 = getCoordinatePoint(mRadius, mStartAngle);//扇形半径  // 起始角度
        // 由结束角度确定的高度
        float[] point2 = getCoordinatePoint(mRadius, mStartAngle + mSweepAngle); //扇形半径  // 起始角度  // 绘制角度
        // 取最大值
